...
**WSL2 → Hyper‑V 本番サーバー** へのデプロイは  
`deploy.sh` により完全自動化されており、  
アプリケーションコードは `/opt/price-monitor/app` に配置されます。
...
### 🔮 将来の改善ロードマップ
- Yahooショッピング側の仕様変更を検知する監視フローの整備  
- User-Agent / HTTP ヘッダーのローテーションによるアクセス安定化  
- スクレイピング失敗時の自動リトライ  
- 本番・開発の設定分離（.env.template の導入）
